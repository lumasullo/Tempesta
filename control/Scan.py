...
# This class is intended as a widget in the bigger GUI, Thus all the commented
# parameters etc. It contains an instance of stage_scan and pixel_scan which in
# turn harbour the analog and digital signals respectively.
# The function run is the function that is supposed to start communication with
# the Nidaq through the Scanner object. This object was initially written as a
# QThread object but is not right now.
...
class ScanWidget(QtGui.QFrame):
    ...
    def __init__(self, device, *args, **kwargs):
        ...
        def saveScanFcn():
            return guitools.saveScan(self)
        self.saveScanBtn.clicked.connect(saveScanFcn)
        self.loadScanBtn = QtGui.QPushButton('Load Scan')

        def loadScanFcn():
            return guitools.loadScan(self)
        self.loadScanBtn.clicked.connect(loadScanFcn)

        self.sampleRateEdit = QtGui.QLineEdit()

        def changeWidth():
            return self.ScanParameterChanged('width')
        self.widthPar = QtGui.QLineEdit('10')
        self.widthPar.editingFinished.connect(changeWidth)

        def changeHeight():
            return self.ScanParameterChanged('height')
        self.heightPar = QtGui.QLineEdit('10')
        self.heightPar.editingFinished.connect(changeHeight)

        def changeSecTime():
            return self.ScanParameterChanged('sequence_time')
        self.sequence_timePar = QtGui.QLineEdit('100')  # Milliseconds
        self.sequence_timePar.editingFinished.connect(changeSecTime)
        self.nrFramesPar = QtGui.QLabel()
        self.scanDuration = QtGui.QLabel()
        self.step_sizePar = QtGui.QLineEdit('0.5')

        def changeStep():
            return self.ScanParameterChanged('step_size')
        self.step_sizePar.editingFinished.connect(changeStep)
...
#        self.sample_rate = np.float(self.sampleRateEdit.text())
        ...
        self.current_dochannels = {'TIS': 0, '355': 1, '405': 2, '488': 3,
                                   'CAM': 4}
        ...
        self.DOchan_Pars_dict = {'TIS': self.chanTISPar,
                                 '355': self.chan355Par,
                                 '405': self.chan405Par,
                                 '488': self.chan488Par,
                                 'CAM': self.chanCAMPar}
...
#        grid.setRowMamimumHeight(1, 20)
    ...
    def PixelParameterChanged(self, parameter):
        ...
        self.pixel_cycle.update([device], self.pixel_par_values,
                                self.stage_scan.sequence_samples)
    ...
    def PreviewScan(self):
        ...
        plt.axis([-0.2,
                  self.scan_par_values['width'] + 0.2, -0.2,
                  self.scan_par_values['height'] + 0.2])
    ...
    def PrepAndRun(self):
        ...
        if self.scanRadio.isChecked():
            ...
            self.scanner = Scanner(self.nidaq, self.stage_scan,
                                   self.pixel_cycle, self.current_aochannels,
                                   self.current_dochannels, self)
...
#            self.lasercycle = LaserCycle(self.pixel_cycle, self.current_dochannels)
            self.lasercycle = LaserCycle(self.nidaq, self.pixel_cycle,
    ...
    def update_Scan(self, devices):
        self.stage_scan.update(self.scan_par_values)
        self.pixel_cycle.update(devices, self.pixel_par_values,
                                self.stage_scan.sequence_samples)
...
class Scanner(QtCore.QObject):
    ...
    def __init__(self, device, stage_scan, pixel_cycle, current_aochannels,
...
#        Following code should correct channels mentioned in Buglist.
        ...
        for i in range(0, 2):
            ramp_and_k = make_ramp(final_samps[i], 0,
                                   self.stage_scan.sample_rate)
...
#        magic = np.ones(100)
    ...
    def runScan(self):
        ...
        for i in range(0, 2):
            ...
            self.aotask.create_voltage_channel(phys_channel=chanstring,
                                               channel_name='chan%s' % dim,
                                               min_val=min_ao,
                                               max_val=max_ao)
...
#        print(np.ones(1)) # This line alone fixes the problem...
        ...
        self.dotask.write(full_do_signal, layout='group_by_channel',
                          auto_start=False)
        self.aotask.write(full_ao_signal, layout='group_by_channel',
                          auto_start=False)
...
# Following class containg the analog signals in sig_dict. The update function
# takes the parameter_values and updates the signals accordingly
class StageScan():
...
class Line_scan():
    ...
    def update(self, par_values, prim_scan_dim):
        ...
        # ramp_and_k contains [ramp, k]
        ramp_and_k = make_ramp(start_y, size_y, column_samples)
        ramp = ramp_and_k[0]

        self.sig_dict[prim_scan_dim+'_sig'] = 1.14 * ramp
        zeros = np.zeros(len(ramp))
        self.sig_dict[chr(121-(ord(prim_scan_dim) % 2)) + '_sig'] = zeros
...
class FOV_Scan():
    ...
    def __init__(self, sample_rate):
        ...
        # slighly from expected scan. Maybe either limit input parameters to
        # numbers that "fit each other" or find other solution, eg step size
        # has to be width divided by an integer.
        # Maybe not a problem ???
    def update_frames(self, par_values):
        step_size = par_values['step_size'] / 2
    ...
    def update(self, par_values, prim_scan_dim):
        ...
        self.sequence_samples = int(np.round(self.sample_rate * par_values['sequence_time']))
        ...
        # ramp_and_k contains [ramp, k]
        ramp_and_k = make_ramp(start_x, size_x, row_samples)
        k = ramp_and_k[1]
        ...
        first_part =  max_turn - turn_rtl[range(int(np.ceil(self.sequence_samples/2)), self.sequence_samples)]
        ...
        for i in range(0, self.steps_y):
            if i % 2 == 0:
                ...
                prim_dim_sig = np.concatenate((prim_dim_sig, rtl_ramp,
                                               turn_ltr))
            sec_dim_sig = np.concatenate((sec_dim_sig,
                                          new_value*np.ones(row_samples),
...
# The follwing class contains the digital signals for the pixel cycle. The
# update function takes a parameter_values dict and updates the signal
# accordingly.
class PixelCycle():

    def __init__(self, sample_rate):
        self.sig_dict = {'TISsig': [], '355sig': [], '405sig': [],
                         '488sig': [], 'CAMsig': []}
        self.sample_rate = sample_rate
