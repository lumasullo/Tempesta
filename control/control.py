...
#        buttonGrid.addWidget(self.snapHDFButton, 0, 1)
        ...
        # Graphically adding the labels and fields etc to the gui. Four numbers
        # specify row, column, rowspan and columnspan.
...
# stop" when recording.
    ...
    def getAttrs(self):
        ...
        attrs.append(
            ('Scan mode', self.main.scanWidget.Scan_Mode.currentText()))
        attrs.append(
            ('True_if_scanning',
...
class RecWorker(QtCore.QObject):
    ...
    def start(self):
        ...
        if last_f is None:
            # If camera has not recorded any frames yet,
        ...
        print('File opened')
        self.store_file.create_dataset(
            name=self.dataname,
            shape=datashape,
            maxshape=datashape,
            dtype=np.uint16)
...
class CamParamTree(ParameterTree):
    ...
    def __init__(self, orcaflash, *args, **kwargs):
        ...
        BinTip = (
            "Sets binning mode. Binning mode specifies if and how many \n"
            "pixels are to be read out and interpreted as a single pixel \n"
            "value.")
    ...
    def writable(self, value):
        ...
        # WARNING: If Apply and New ROI button are included here they will emit
...
class TormentaGUI(QtGui.QMainWindow):
    ...
    def __init__(self, violetlaser, exclaser, offlaser, orcaflashV2,
        ...
        # 1:OFF, 2:ON
#        self.changeParameter(
#           lambda: self.orcaflash.setPropertyValue('defect_correct_mode', 2))
#        self.orcaflash.setPropertyValue('readout_speed', 1)

#        self.changeParameter(
#           lambda: self.orcaflash.setPropertyValue('trigger_mode', 6))
        ...
        self.curr_cam_ind = 0
        self.latest_images = [np.zeros(self.shapes[self.curr_cam_ind]),
                              np.zeros(self.shapes[self.curr_cam_ind])]
...
#        self.hist.vb.setLimits(yMin=0, yMax=2048)
        ...
        self.alignmentLineMakerButton = QtGui.QPushButton(
            'Make Alignment Line')
    ...
    def mouseMoved(self, pos):
        if self.vb.sceneBoundingRect().contains(pos):
            ...
            x, y = int(mousePoint.x()), int(
                self.shapes[self.curr_cam_ind][1] - mousePoint.y())
...
#            self.EffFRPar.Enable(True)
            ...
            self.changeParameter(
                lambda: self.cameras[self.curr_cam_ind].setPropertyValue(
                    'trigger_mode', 6))
            print(self.cameras[self.curr_cam_ind].getPropertyValue(
                'trigger_mode'))
...
#            self.EffFRPar.Enable(False)
    ...
    def setBinning(self):
        ...
        self.changeParameter(
            lambda: self.orcaflash.setPropertyValue(
                'binning', coded))
...
#       Round to closest "divisable by 4" value.
        ...
        self.cameras[self.curr_cam_ind].setPropertyValue(
            'subarray_vsize', 2048)
        self.cameras[self.curr_cam_ind].setPropertyValue(
            'subarray_hsize', 2048)
        ...
        self.shapes[self.curr_cam_ind] = (hsize, vsize)
    ...
    def adjustFrame(self):
        ...
        self.changeParameter(
            lambda: self.cropOrca(
                binning * self.X0par.value(),
    ...
    def AbortROI(self):
        ...
        self.Widthpar.setValue(
            self.shapes[self.curr_cam_ind][0])  # [0] is Width
        self.Heightpar.setValue(
            self.shapes[self.curr_cam_ind][1])  # [1] is Height
...
#            self.orcaflash.getPropertyValue('exposure_time')[0])
#        RealAccPar.setValue(
#            self.orcaflash.getPropertyValue('accumulation_time')[0])
#        EffFRPar.setValue(
#            1/self.orcaflash.getPropertyValue('accumulation_time')[0])
    ...
    def liveviewStop(self):
        ...
        self.img.setImage(
            np.zeros(self.shapes[self.curr_cam_ind]), autoLevels=False)
...
#        print('In update view')
        self.img.setImage(self.latest_images[self.curr_cam_ind],
